...
stop(_)    -> ok.
spec()     ->
    Acceptors  = application:get_env(review, acceptors,   4),
    Clients    = application:get_env(review, max_clients, 512),
    Protocol   = application:get_env(review, protocol,    http),
    Port       = application:get_env(review, port,        8000),
...
docroot() ->
    {file, Here} = code:is_loaded(review),
    Dir = filename:dirname(filename:dirname(Here)),
    Root = application:get_env(review, "statics_root", "priv/static"),
